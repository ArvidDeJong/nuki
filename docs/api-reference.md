...
| `nuki:user-create` | Create a main `NukiUser` for the package's own auth guard. Options: `--email`, `--name`, `--password` (asked for when left out), `--account=<account_key>` (repeatable, since 1.3.0: attaches the user as owner to an existing account; an unknown key is an error and nothing is created) and `--no-2fa`, which stores `two_factor_enabled = false`; that column is not read at login, so the user still gets a code while `auth_users.otp.enabled` is `true`. See [NukiUserCreateCommand](https://github.com/ArvidDeJong/nuki/blob/main/src/Console/Commands/NukiUserCreateCommand.php) and [Users and permissions](users-and-permissions.md). |
