...
- `Nuki::as('some-account')` returns a clone scoped to the token stored under that account key. That is the multi account path in both modes: with `auth` set to `token` and the `database` token resolver (the default) every row in `nuki_accounts` is an account, with `auth` set to `oauth` every stored OAuth token is. Only the `config` token resolver knows one account, `default`.
...
- A failing call throws `ApiException` (`->status`, `->body` as a raw string, `->endpoint`); an authentication problem throws `AuthenticationException`. Both extend `NukiException`. A server that cannot be reached surfaces as Laravel's `Illuminate\Http\Client\ConnectionException`, which does **not**, so catch `NukiException|ConnectionException` when a timeout must not break the page.
...
- `auth_users` is off by default. Switching it on registers its own `darvis-nuki` auth guard with the `NukiUser` model, puts every bundled route behind it, and enables sub users with per smartlock permissions. A guest of the package pages is redirected to `/nuki/login` by the package's own `AuthenticateNukiUser` middleware (since 1.3.0; don't add a `redirectGuestsTo()` workaround for it). A package user only sees the `nuki_accounts` rows they are attached to: the main user who creates an account on `/nuki/accounts` is attached as `owner`, `nuki:user-create --account=<key>` attaches a new user, and anything else takes `$user->accounts()->syncWithoutDetaching([...])`. A sub user only sees their own locks, on the dashboard and the timeline too, and logs only with `view_logs`. **Those permissions are local to your application; nothing is written back to the NUKI account.** Self registration (`auth_users.register_enabled`) is off by default because a registered account is a main user who may operate every lock; create users with `php artisan nuki:user-create`. The accounts and webhooks pages are for a main user only.
