...
- Webhooks arrive on the route from `webhook.route`. The body is checked with HMAC-SHA256 against `webhook.secret`; **without a secret the check is skipped**, so set one. Act on a webhook by listening for `Darvis\Nuki\Events\NukiWebhookReceived` (`type`, `payload`, `accountKey`), never by adding your own controller to that route.
