...
- A guest on a package page is redirected to `/nuki/login` (route `nuki.auth.login`) by `Darvis\Nuki\Http\Middleware\AuthenticateNukiUser`, and a signed in package user who opens a guest page goes to `auth_users.redirect_after_login` (`RedirectIfNukiUser`). Both since 1.3.0; before that Laravel's bare `auth:darvis-nuki` sent a guest to the host application's `login` route, or failed with `Route [login] not defined.` The host application's `redirectGuestsTo()` is not touched. Use `AuthenticateNukiUser::class` on your own routes for package users.
- A package user only sees and switches to the `nuki_accounts` rows they are attached to, plus `default`. Since 1.3.0 the main user who creates an account on `/nuki/accounts` is attached to it as `owner` (`NukiAccount::ROLE_OWNER`), and `php artisan nuki:user-create --account=<account_key>` (repeatable) attaches a new main user to existing accounts; an unknown key fails and creates nothing. Everything else is `$user->accounts()->syncWithoutDetaching([$account->id => ['role' => 'owner']])`. Sub users inherit the accounts of their parent.
- For a sub user the dashboard counts and lists only their own locks, and the dashboard activity and `/nuki/activity` only have entries of locks with `view_logs`; `/nuki/activity?lock=<id>` for any other lock is a `403`. The sub user editor only offers the accounts the main user is attached to.
