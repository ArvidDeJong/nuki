<div>
    ...
    <form wire:submit="submit" class="mt-8 space-y-6">
        <flux:field>
            <flux:label>{{ __('nuki::nuki.auth.name') }}</flux:label>
        ...
        <flux:field>
            <flux:label>{{ __('nuki::nuki.auth.confirm_password') }}</flux:label>
        ...
        <div class="flex items-center justify-between gap-3 pt-2">
            <flux:button type="submit" variant="primary">{{ __('nuki::nuki.auth.create_account') }}</flux:button>
            <flux:link href="{{ route('nuki.auth.login') }}" variant="ghost">{{ __('nuki::nuki.auth.login') }}</flux:link>
        </div>
    </form>
</div>
